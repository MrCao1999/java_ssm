...
@WebServlet("/studentSign/page")
public class StudentSignPage extends HttpServlet {
	private static final long serialVersionUID = 1L;
	static Logger log = LoggerFactory.getLogger(StudentSignPage.class);
	@Autowired
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(pageNum == null) {
			...
			Page<StudentSign> page = studentSignService.list(studentSign, page_Size, page_num);
			request.setAttribute("page", page);
			request.setAttribute("studentSign", studentSign);
			request.getRequestDispatcher("/jsp/studentSign/index.jsp").forward(request, response);
		}else {
			if (pageSize != null || !(Objects.equals("", pageSize))) {
				page_Size = Integer.valueOf(page_Size);
			}
